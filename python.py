...
def login():
    if 'user_id' in session:
        user = User.query.filter_by(id=session['user_id'], deleted_at=None).first()
        if user:
            print(f"Session already exists: {dict(session)}")
    ...
    if request.method == 'POST':
        try:
            ...
            print(f"Session set after login: {dict(session)}")
            return redirect(url_for('dashboard'))
...
def profile():
    ...
    try:
        ...
        return render_template('error.html', message="An error occurred while fetching your profile.")
...
def team_overview():
    try:
        ...
        return render_template('error.html', message="An error occurred while fetching team details.")
...
def assign_project():
    if request.method == 'GET':
        try:
            ...
            return render_template('error.html', message="An error occurred while loading the project assignment page.")
    ...
    try:
        ...
        if not title or not description or not emp_id:
...
def chat():
    ...
    return render_template('chat.html', users=users, current_user_id=current_user_id)
